...
| 6 | ☐ | IX-ULWCD を手元に用意する(所有品、借用、または購入)。そのうえで M6(外径・外形)と M8(ピント状態の前玉高さ)を測る。ホルダーの試作品か購入品で M23(コレット先端 → ヘッド最上点の鉛直距離)も測る。V1 プレート Corning 7007 の実物で M19 も測る | 凍結ゲートの残り項目。アームの細い部分の長さ、コンデンサとの余裕 | `05` M6/M8 |
...
> **凍結ゲート(10 項目): M1, M3, M4, M6, M7, M8, M10, M15, M19, M23**
>
> この 10 項目を実測して `cad/params.py` に入れ、再計算するまで、ブラケット形状、部品型番、加工図を確定しません(`05_measurement_checklist.md`、`07_assumptions_and_open_items.md`)。W-B では、選んだ電動 XY ステージの外形と移動量もあわせて記録します(`05` 冒頭、`09` 凍結ゲート表)。
>
...
| ☐ | I-4 | V1 は Corning 7007 に固定。ほかに使いたいプレートがあれば種類と型番を挙げる(試験扱い)。蓋を置く場所(M21) | M19, M21。角度ブロックの組合せ、安全高さ | なし | 担当: ___ / 時期: ___ | `05` M19/M21, `10` |
...
| ☐ | **M19** | A | V1 プレート Corning 7007 の実物のウェル縁径、深さ、蓋なし高さ、A1 の位置(ほかのプレートは試験扱いで記録のみ) | 購入ロットをノギスとデプスゲージで。メーカー図面と比べる | V1 の受け入れ条件(目標が底中心から 0.5 mm 以内)、安全高さの下限 | I-4 | 担当: ___ / 時期: ___ | `05`, `10` |
...
|---|---|---|---|---|---|---|
| ☐ | C-1 | 凍結ゲートの 10 項目がすべて `MEAS` になっていることを確認し、`python tools/build_all.py` を **`--fast` なしで**実行する(干渉スイープを含む) | 実測値での干渉結果を得る | 凍結ゲートの実測 | 担当: ___ / 時期: ___ | README §Rebuild |
...
| ☐ | C-3 | 残っている仮値(`PH`)の一覧を見て、残してよいものか確認する。現在は 19 件(手順書の「まだ仮の値」一覧) | 見落としの防止 | C-1 | 担当: ___ / 時期: ___ | 下の付録, 手順書 |
...
| `HEAD_TOP_FROM_NOSE_MEAS`(コレット先端 → ヘッド最上点の鉛直距離。未入力なら `HOLDER_AXIAL_LEN` などから計算) | 22.2 mm(計算値) | **M23** |
