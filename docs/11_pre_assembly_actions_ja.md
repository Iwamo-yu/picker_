<!-- GENERATED from docs/src/11_pre_assembly_actions_ja.md by cad/render_docs.py - edit the source, not this file -->
...
| 3 | ☐ | 手元のポンプの型式、ポート、通信方式を確認する(I-3, M17) | シリンジ容量、チューブ、D6 | `05` M17, `08` |
...
| 6 | ☐ | IX-ULWCD を手元に用意する(所有品、借用、または購入)。そのうえで M6(外径・外形)と M8(ピント状態の前玉高さ)を測る | 凍結ゲートの残り 2 項目。アームの細い部分の長さ | `05` M6/M8 |
...
> **凍結ゲート: M1, M3, M4, M6, M7, M8, M10, M15**
>
> この 8 項目を実測して `cad/params.py` に入れ、再計算するまで、ブラケット形状、部品型番、加工図を確定しません(`05_measurement_checklist.md`、`07_assumptions_and_open_items.md`)。W-B では、選んだ電動 XY ステージの外形と移動量もあわせて記録します(`05` 冒頭、`09` 凍結ゲート表)。
>
> 現在のモデルのコンデンサ付近の余裕(安全高さで 4.83 mm)は、仮のコンデンサ外形(Ø80 mm)から出た値です。**設計根拠ではありません。**
...
| ☐ | I-4 | 使うプレートの種類と型番をすべて挙げる(U 底 ULA、平底、マイクロキャビティ等)。蓋を置く場所(M21) | M19, M21。角度ブロックの組合せ、安全高さ | なし | 担当: ___ / 時期: ___ | `05` M19/M21, `10` |
...
| ☐ | M17 | B | ポンプの外形、チューブポート、制御方式 | I-3 と同時に。データシートも入手 | ポンプ位置、流路長、チューブ(O-6) | I-3 | 担当: ___ / 時期: ___ | `05` |
| ☐ | M19 | C | 使うプレートの深さとウェル縁径 | メーカー図面を優先。なければノギスで | 角度の余裕、角度ブロックの組合せ | I-4 | 担当: ___ / 時期: ___ | `05`, `10` |
...
|---|---|---|---|---|---|---|
| ☐ | C-1 | 凍結ゲートの 8 項目がすべて `MEAS` になっていることを確認し、`python tools/build_all.py` を **`--fast` なしで**実行する(干渉スイープを含む) | 実測値での干渉結果を得る | 凍結ゲートの実測 | 担当: ___ / 時期: ___ | README §Rebuild |
...
| ☐ | C-3 | 残っている仮値(`PH`)の一覧を見て、残してよいものか確認する。現在は 13 件(手順書の「まだ仮の値」一覧) | 見落としの防止 | C-1 | 担当: ___ / 時期: ___ | 下の付録, 手順書 |
...
| ☐ | C-8 | 安全の準備: 非常停止の配線計画、Z 原点が上端であること、Z が電源断で落ちないことを組立時に確認する段取り | 手順書「安全の決まりごと」の前提 | O-7 | 担当: ___ / 時期: ___ | `06`, 手順書 |
...
| `STAGE_T`(ステージ板厚) | 20 mm | 対応する M 番号なし(M3 と一緒に測るのが自然) |
| `BODY_Y_FRONT` | −235 mm | M5 |
| `STAGE_CENTER_Y` | 0 mm | **M4** |
| `PILLAR_W`, `PILLAR_Y0`, `PILLAR_Y1` | 90 / 150 / 240 mm | M9 |
| `COND_ARM_W` | 90 mm | **M7** |
| `COND_D` | 80 mm | **M6** |
| `COND_BODY_H`(コンデンサ本体高さ) | 70 mm | 対応する M 番号なし(M6 と一緒に測るのが自然) |
| `OBS_TUBE_BOX` | 箱形状 | M12 |
| `STAGE_HANDLE` | 箱形状 | M13 |
| `OBJECTIVE_ZONE` | r 45 mm | M14(params のメモには M 番号なし) |

M8(コンデンサ前玉の実高さ)は、`params.py` ではメーカーの WD から計算しているため PH としては数えられていません。M10(支柱を倒した外形)と M17(ポンプ)に対応するパラメータは `params.py` にありません。どちらも凍結ゲートや発注に効くので、測った値はこの文書か `05` に記録してください。

---

## 確認中の食い違い(リポジトリ内)

この計画に影響するものだけを挙げます。どちらが正しいかは未確定です。

1. **コンデンサとの余裕の判定基準**: 組立手順書の工程 7 は「余裕が 5 mm を切るならアームを薄くするか、照明支柱を倒す案に切り替える」としています。一方、`10` と `params.py` の `MIN_MARGIN` は 2 mm を合格基準としています。現在のモデル値 4.83 mm は前者では不合格、後者では合格です。C-2 の前に基準を決めてください。
2. **角度の調整方法**: `03` §6・§9 は「0–12° の円弧スロットで一度だけ調整」、`10` と `params.py` のコメントは「交換式の角度ブロック(8/20/30°)」です。この計画は後者(F-6)に従っています。D3 の意味も「ブロックの角度を選ぶ」ことになります。
3. **凍結ゲートの項目数**: `03` と `09` の本文は「M1, M6, M7, M8」の 4 項目、`05`・`07`・README・`09` の凍結ゲート表は 8 項目です。この計画は 8 項目に従っています。
4. **直動ステージのストローク**: `04` の推奨は Z 55–80 mm、X 150 mm(W-A の値)で、W-B の X 100 / Y 30 / Z 50 mm と合っていません。O-4 は W-B の値で見積もってください。
