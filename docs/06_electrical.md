...
- Z: **two switches**.
  - The **reference switch** sits at the carriage height where the head top (arm plus tubing) is 2 mm below the condenser front. It is set from M8 at installation. At that height the arm clears the condenser at every XY, and the tip is above safe-Z for every plate setting in `10_…`: tip at 22.2 / 24.4 / 27.6 mm above the stage top for the 8° / 20° / 30° blocks.
