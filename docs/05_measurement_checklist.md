...
**Freeze gate (10 items):** M1, M3, M4, M6, M7, M8, M10, M15, M19, M23 must be measured before any bracket or part number is frozen (`09_workflow_D1.md`). For W-B, also record the chosen motorised stage's envelope and travel.
...
| M19 | A | **V1 plate lot check**: the plates actually bought are Corning 7007 (96-well U-bottom); measure well rim Ø, depth, plate height without lid, and A1 corner position in the holder. Other plate types are experimental and recorded only | Corning 7007 manufacturer drawing | V1 acceptance (target radius 0.5 mm), angle margin, safe-Z lower bound |
...
| M23 | A | **Head height above the nose** (`HEAD_TOP_FROM_NOSE_MEAS`): with the real V1 holder (collet, nut, depth stop, seal, side port) mounted on the 8° block and the tubing clipped, the **vertical** distance from the collet nose to the highest point of the head | 22.2 mm = holder axial length 14 mm × cos θ + half the arm + 2.3 mm tubing (APX) | condenser margin and the safe-Z corridor upper bound fall 1 mm per mm; limit over the recommended settings ≈ 22.6 mm (`10_…`) |
