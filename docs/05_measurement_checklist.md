...
**Freeze gate:** M1, M3, M4, M6, M7, M8, M10, M15, M23 must be measured before any bracket or part number is frozen (`09_workflow_D1.md`). For W-B, also record the chosen motorised stage's envelope and travel.
...
| M23 | A | **Holder stack height**: with the real holder (collet, nut, depth stop, seal, side port) mounted on its angle block, the distance from the collet nose to the highest point of the head, including the tubing clip | `HOLDER_L` 14 mm + arm + 2.3 mm tubing | condenser margin falls about 1 mm per mm; limit ≈ 15.8 mm (`10_…`) |
